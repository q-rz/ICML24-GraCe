...
class Optimizer(BaseOptimizer):
    ...
    def init(self):
        ...
        self.perm = np.random.permutation(self.d)
        self.n_blocks = args.n_blocks
        self.block_size = (self.d + self.n_blocks - 1) // self.n_blocks # ceil(self.d / self.n_blocks)
        self.s_block = math.ceil(1.1 * self.s / self.n_blocks)
        self.Z = 2 * (np.random.rand(args.num_samples, self.block_size) > 0.5) - 1 # ZO-BCD-R
    def step(self):
        '''
        @link https://github.com/YuchenLou/ZO-BCD/blob/main/pyZO-BCD/optimizers.py
        '''
        delta = args.eps
        num_samples = args.num_samples
        block_start = np.random.randint(self.n_blocks) * self.block_size
        block = self.perm[block_start : min(block_start + self.block_size, self.d)]
        Z_padded = np.zeros((num_samples, self.d))
        Z_padded[:, block] = Z = self.Z[:, : block.shape[0]]
        x = self.x
        y0 = self.y
        f = self.bm.query1
        ...
        grad_estimate = np.zeros(self.d)
        grad_estimate[block] = cosamp(Phi = Z, u = y, K = self.s_block, tol = args.cosamp_tol, maxiterations = args.cosamp_maxiterations)
